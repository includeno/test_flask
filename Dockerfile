...
RUN /bin/sh -c 'cd /app && git clone https://github.com/includeno/test_flask.git'
...
RUN /bin/sh -c 'cd /app && ls '

RUN /bin/sh -c 'cd /app/test_flask && python -m pip install -r requirements.txt '
...
CMD ["python", "/app/test_flask/flaskmain.py"]
